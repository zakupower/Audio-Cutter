...
/**
 * Returns the audio standard deviation, which is <tt>Math.sqrt(variance())</tt>.
 */
public synchronized double standardDeviation() {
	return Math.sqrt(variance());
}
/**
 * Returns the audio standard error, which is <tt>Math.sqrt(variance() / size())</tt>
 */
...
/**
 * Returns the audio variance, which is <tt>Sum( (x[i]-mean())<sup>2</sup> )  /  (size()-1)</tt>.
 */
