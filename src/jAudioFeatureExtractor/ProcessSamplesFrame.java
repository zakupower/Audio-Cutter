 ...
 * audio has an amplitude corresponding to the maximum value allowed by the bit
 * depth.
 * <p>
 ...
 * End fields between time and audio values. These affect the sections of the
 ...
 * <p>
 * The View Samples button displays the audio values in the region entered in
 ...
 * the audio values for each channel between the limits given in the Start and
...
public class ProcessSamplesFrame extends JFrame implements ActionListener {
	...
	/**
	 * Normalize the samples so that the absolute value of the largest audio
	 * value is 1.
	 */
	...
	/**
	 * Display a dialog box showing the audio values for each separate channel
	...
	/**
	 * Display a dialog box showing a plot of the audio values for each
	...
	/**
	 * Display a dialog box showing a plot of the power spectrum of the audio
	 ...
	 * between audio indices and times. If the reset parameter is true, then
	...
	private void convertStartEndFields(boolean reset) {
		...
		// If switching between time and audio indice, or vice versa
		else {
	...
	private void verifyStartAndEndFields() throws Exception {
		if (time_start_end_type_radio_button.isSelected()) {
			...
			if (start_sample < 0 || start_sample > maximum_sample)
				throw new Exception("Start audio is specified to be "
						+ start_sample + ".\n"
						+ "This value must be between 0 and " + maximum_sample
						+ ".");
			if (end_sample <= start_sample)
				throw new Exception("End audio is " + end_sample
						+ " and start audio is " + start_sample + ".\n"
						+ "Start audio must be after end audio.");
			if (end_sample > maximum_sample)
				throw new Exception("End audio of " + end_sample
