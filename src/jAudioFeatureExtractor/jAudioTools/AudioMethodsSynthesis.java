...
public class AudioMethodsSynthesis
{
	 ...
	 * <code>AudioFormat</code>. Alternatively, returns the audio values in the form
	 ...
	 *									indicates audio value. In stereo, indice
		...
		throws Exception
	{
		...
		// Generate the appropriate audio values for the given synthesis_type,
	 ...
	 */
		...
		throws Exception
	 {
		...
		if (sample_values == null)
			throw new Exception( "Empty set of samples provided." );
		if (sample_rate <= 0.0F)
			throw new Exception( "Given audio rate is " + sample_rate + " Hz.\n" +
		...
		// Find audio to start and stop envelopes
		...
		throws Exception
	{
		...
		// Find the maximum value a audio may have under the current bit depth
	...
	/**
	 * Generates audio values for a tone consisting of a single sinusoid.
	 * An identical signal is provided to each channel.
	 *
	 * <p>This is an example of a standard audio generating method. All
	 ...
	 * whose second incice corresponds to audio value. In mono, there
		...
		throws Exception
	{
		...
		{
			// Find the time of the current audio
			double time = samp / sample_rate;

			// Add sinusoids to get basic audio value
	...
	/**
	 * Generates audio values for a tone consisting of a sinusoid at a
		...
		throws Exception
	{
		...
		for (int samp = 0; samp < total_samples_per_chan; samp++)
		{
			...
			// Normalize audio values so that they fall between -1 and +1
	...
	/**
	 * Generates audio values for a sound consisting of a stereo sweep, starting
	...
	/**
	 * Generates audio values for a sound consisting of a stereo pinpong, where
	...
	/**
	 * Generates audio values for an audio wave consisting of randomly
